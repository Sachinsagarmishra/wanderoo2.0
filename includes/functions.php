...
try {
    // 1. Backward compatibility: Rename about_settings to site_settings if exists
    ...
    if ($table_check && !$site_table_check) {
        $pdo->exec("RENAME TABLE `about_settings` TO `site_settings` CASCADE;");
    }
    ...
    // 3. Prepare all default settings
    $default_settings = [
        // ABOUT HERO
    ...
    // Seed missing items in site_settings
...
function get_site_setting($key, $default = '') {
    global $pdo, $site_settings_cache;
